...
def main():
    ...
    default_fps = None
    video_paths = data_path.glob("*.avi")
    ...
    for video_index, video_path in enumerate(video_paths):
        ...
        file_path = Path(data_path, video_path)
        ...
        if default_fps is None:
            default_fps = fps
        if fps != default_fps:
            print(f'Warning : {file_path} FPS is different : {fps} instead of {default_fps}')
            continue
        frame_count = 0
        while True:
            ...
            img_output_path = output_path / (str(video_path)[:-4] + f'_{frame_count}.jpg')
            if not img_output_path.parent.exists():
                os.makedirs(str(img_output_path.parent), exist_ok=True)
